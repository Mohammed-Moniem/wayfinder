...
- Once the state can render truthfully, start the installed dashboard with `dashboard --interactive` using the available persistent process runner and give the user its exact temporary loopback URL. Do not merely recommend that they run it later.

Chat and dashboard share canonical state. Refresh before continuing after a dashboard answer; never duplicate or overwrite a newer receipt.
...
- Require later specification, tickets, and implementation to cite the current decision IDs and revisions they follow. Contradicting delivery evidence triggers targeted revalidation, not a silent route change.
